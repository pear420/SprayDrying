...
% Derivatives:
X_betaTRANSPORT = -kc_beta*a*(X_beta-X_beta_INTERPHASE)/(1-epsi);
X_betaCONV = -G_beta*(X_beta-X_beta_INLET)/rho_beta/(1-epsi)/V;
dX_beta_dt = X_betaTRANSPORT+X_betaCONV;

X_gammaTRANSPORT = kc_gamma*a*(X_gamma_INTERPHASE-X_gamma)/epsi;
X_gammaCONV= -G_gamma*(X_gamma-X_gamma_INLET)/rho_gamma/epsi/V;
dX_gamma_dt = X_gammaTRANSPORT+X_gammaCONV;

T_betaTRANSPORT = h_beta*a*(T_INTERPHASE-T_beta)/(rho_beta*(1-epsi)*(Cp_beta+Cp_w*X_beta));
T_betaMAT = -Cp_w*T_beta/(Cp_beta+Cp_w*X_beta)*dX_beta_dt;
T_betaCONV = -G_beta*((Cp_beta+Cp_w*X_beta)*T_beta-(Cp_beta+Cp_w*X_beta_INLET)*T_beta_INLET)/(rho_beta*(1-epsi)*V*(Cp_beta+Cp_w*X_beta));
dT_beta_dt = T_betaTRANSPORT+T_betaMAT+T_betaCONV;

T_gammaTRANSPORT = -h_gamma*a*(T_gamma-T_INTERPHASE)/(rho_gamma*epsi*(Cp_gamma+Cp_wv*X_gamma));
T_gammaMAT = +kc_beta*a*rho_beta*(X_beta-X_beta_INTERPHASE)*DHev_w/(rho_gamma*epsi*(Cp_gamma+Cp_wv*X_gamma));
T_gammaCONV = -(H_wv+Cp_wv*T_gamma)/(Cp_gamma+Cp_wv*X_gamma)*dX_gamma_dt-1/(epsi*rho_gamma*V*(Cp_gamma+Cp_wv*X_gamma))*(G_gamma*((Cp_gamma*T_gamma+(H_wv+Cp_wv*T_gamma)*X_gamma)-(Cp_gamma*T_gamma_INLET+(H_wv+Cp_wv*T_gamma_INLET)*X_gamma_INLET)));
dT_gamma_dt = T_gammaTRANSPORT+T_gammaMAT+T_gammaCONV; % -h_out*A_out*(T_gamma-T_out)/(epsi*rho_gamma*V*(Cp_gamma+Cp_wv*X_gamma));
